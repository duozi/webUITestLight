...
class marketing_WeChat_strategy(unittest.TestCase):
    ...
    def test_marketing_WeChat_strategy(self):
        driver = self.driver
        if self.dict_data["操作类型"].encode('utf8') == '新增微信规则流程':
            self.logstu.info(u"新增微信规则流程")
            self.new_WeChat_rule()
            time.sleep(2)
        if self.dict_data["操作类型"].encode('utf8') == '微信规则修改':
            self.logstu.info(u"微信规则修改")
            self.WeChat_rule_modification()
            time.sleep(2)
        if self.dict_data["操作类型"].encode('utf8') == '微信规则删除':
            self.logstu.info(u"微信规则删除")
            self.del_WeChat_rule()
            time.sleep(2)

    def del_WeChat_rule(self):
        driver = self.driver
    ...
    def WeChat_rule_modification(self):
        driver = self.driver
        self.pub.comeinifm('right')
        self.pub.selectAutoSets(self.dict_data)
        time.sleep(2)
