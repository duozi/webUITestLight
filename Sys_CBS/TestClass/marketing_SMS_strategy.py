...
class marketing_SMS_strategy(unittest.TestCase):
    ...
    def test_marketing_SMS_strategy(self):
        driver = self.driver
        if self.dict_data["操作类型"].encode('utf8') == '新增短信规则流程':
            self.logstu.info(u"新增短信规则流程")
            self.new_SMS_rule()
            time.sleep(2)
        if self.dict_data["操作类型"].encode('utf8') == '短信规则修改':
            self.logstu.info(u"短信规则修改")
            self.SMS_rule_modification()
            time.sleep(2)
        if self.dict_data["操作类型"].encode('utf8') == '短信规则删除':
            self.logstu.info(u"短信规则删除")
            self.del_SMS_rule()
            time.sleep(2)

    def del_SMS_rule(self):
        driver = self.driver
    ...
    def SMS_rule_modification(self):
        driver = self.driver
        self.pub.comeinifm('right')
        self.pub.selectAutoSets(self.dict_data)
        time.sleep(2)
