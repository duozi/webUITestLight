...
#create by limeng
#2016.8.16
class ProductConfigCashLoan(unittest.TestCase):
    ...
    def test_ProductConfigCashLoan(self):
        driver = self.driver
        if self.dict_data["操作类型"] == '新增产品-现金贷':
            self.logstu.info(self.dict_data["操作类型"])
            self.new_Cash_Config()
        if self.dict_data["操作类型"] == '新增产品失败-现金贷':
            self.logstu.info(self.dict_data["操作类型"])
            self.new_Cash_Config_fail()
        if self.dict_data["操作类型"] == '修改产品基本信息-现金贷':
            self.logstu.info(self.dict_data["操作类型"])
            self.Cash_Config_info_modification()
        if self.dict_data["操作类型"] == '产品费用配置移除费用-现金贷':
            self.logstu.info(self.dict_data["操作类型"])
            self.Cash_Config_remove_cost_modification()
        if self.dict_data["操作类型"] == '产品费用配置导入费用-现金贷':
            self.logstu.info(self.dict_data["操作类型"])
            self.Cash_Config_Import_cost_modification()
        if self.dict_data["操作类型"] == '产品费用配置导入滞纳金-现金贷':
            self.logstu.info(self.dict_data["操作类型"])
            self.Cash_Config_Import_surcharge_modification()

    def new_Cash_Config(self):
        driver = self.driver
    ...
    def Cash_Config_Import_surcharge_modification(self):
        driver = self.driver
        self.pub.comeinifm('right')
    ...
    def Cash_Config_info_modification(self):
        driver = self.driver
        self.pub.comeinifm('right')
        self.pub.selectAutoSets(self.dict_data)
        time.sleep(2)
    ...
    def Cash_Config_Import_cost_modification(self):
        driver = self.driver
        self.pub.comeinifm('right')
        self.pub.selectAutoSets(self.dict_data)
        time.sleep(2)
    ...
    def Cash_Config_remove_cost_modification(self):
        driver = self.driver
        self.pub.comeinifm('right')
        self.pub.selectAutoSets(self.dict_data)
        time.sleep(2)
