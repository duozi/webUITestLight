...
class ElementFinder(object):
    ...
    def __init__(self):
        ...
        self._default_strategies = list(self.strategies.keys())
    ...
    def find(self, browser, locator, tag=None):
        ...
        strategy = self.strategies.get(prefix) #根据标签获取方法名，参考__init__
        if strategy is None:
            raise ValueError("Element locator with prefix '" + prefix + "' is not supported")
        (tag, constraints) = self._get_tag_and_constraints(tag) #解析标签
        return strategy(browser, criteria, tag, constraints) #调用具体方法
    ...
    def register(self, strategy):
        ...
        self.strategies[strategy.name] = strategy.find
    ...
    def _find_by_key_attrs(self, browser, criteria, tag, constraints):
        ...
        xpath_criteria = self.escape_xpath_value(criteria)
        ...
        for attr in ['@src', '@href']:
            if attr in key_attrs:
                if url is None or xpath_url is None:
                    ...
                    xpath_url = self.escape_xpath_value(url)
